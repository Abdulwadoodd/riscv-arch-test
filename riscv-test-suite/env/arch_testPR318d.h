...
common_\__MODE__\()excpt_handler:
  ...
  // This code calculates this table:
  // Curr Mode  MPP==3  xPV=0   xPV=1
  //      M     0       1       2 *sv_area_sz
  //    S,U     X       0       1 *sv_area_sz (prev priv can't be M if this is S/U)
  //    VS,VU   X       X       0 *sv_area_sz (prev priv must be V if this is V)
  // lvs result in t4 to be used during relocation, ( so doesn't touch sp)
  // can use t3, t6 because relocation will overwrite them
  //********************************************************************************
        mv      t4, sp                  /* default reloc offset to 0*sv_area_sz */
.if (\__MODE__\() != V)                 /* if Vmode, must be horiz trap, no off */
        csrr    t6, CSR_XSTATUS
        slli    t3, t6, XLEN-MPP_LSB-2  /* put mpp msb into msb                 */
        bltz    t3, vmem_adj_\__MODE__\()epc /* mpp!=0, Mmode horiz trap, 0*off */
  .if (\__MODE__\() == M)               /* vert trap into Mmode, min off= *1    */
        addi    t4, t4, sv_area_sz      /* upd offset to be at least sv_area_sz */
  .endif
//   .if (XLEN==32)
//         csrr    t6, CSR_XSTATUSH        /* get half of status with V-bit        */
//   .endif
